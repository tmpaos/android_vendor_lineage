function __print_axiomos_functions_help() {
cat <<EOF
Additional AxiomOS functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- axiomosgerrit:   A Git wrapper that fetches/pushes patch from/to AxiomOS Gerrit Review.
- axiomosrebase:   Rebase a Gerrit change and push it again.
- axiomosremote:   Add git remote for AxiomOS Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for AxiomOS Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the Axiomos model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch axiomos_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/axiomos-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.axiomos.device | grep -q "$AXIOMOS_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $AXIOMOS_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function axiomosremote()
{
    ...
    git remote rm axiomos 2> /dev/null
    ...
    local AXIOMOS_USER=$(git config --get review.review.axiomosos.org.username)
    if [ -z "$AXIOMOS_USER" ]
    then
        git remote add axiomos ssh://review.axiomosos.org:29418/$PFX$PROJECT
    else
        git remote add axiomos ssh://$AXIOMOS_USER@review.axiomosos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'axiomos' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/AxiomOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.axiomos.device | grep -q "$AXIOMOS_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        axiomosremote
        git push axiomos HEAD:refs/heads/'$1'
    fi
    '
}

function axiomosgerrit() {
    ...
    local user=`git config --get review.review.axiomosos.org.username`
...
EOF
            ...
            case $1 in
                ...
                changes|for)
                    if [ "$FUNCNAME" = "axiomosgerrit" ]; then
...
function axiomosrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "AxiomOS Gerrit Rebase Usage: "
        echo "      axiomosrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.axiomosos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.axiomos.device | grep -q "$AXIOMOS_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/axiomos/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $AXIOMOS_FIXUP_COMMON_OUT ]; then
