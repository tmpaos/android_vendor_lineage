...
func init() {
	android.RegisterModuleType("axiomos_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = axiomosExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = axiomosExpandVariables(ctx, dep_file)
	...
	cmd := axiomosExpandVariables(ctx, String(g.properties.Cmd))
